...
11. Asegúrate de que todos los recursos estén ubicados en el grupo de recursos especificado utilizando el recurso 'azurerm_resource_group'.

12. Configura el proveedor de Azure con el atributo 'skip_provider_registration' en 'true' y el bloque 'features'.

Recuerda que este es solo un esquema general y que debes ajustar el código a tus necesidades específicas. Además, es importante tener en cuenta las mejores prácticas de seguridad y cumplir con las políticas de tu organización al trabajar con recursos en la nube.