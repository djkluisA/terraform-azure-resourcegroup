...
resource "tls_private_key" "key" {
  ...
  depends_on = [
    azurerm_key_vault.kvaultmv126052023
  ]
}

resource "azurerm_key_vault" "kvaultmv126052023" {
  name                = "kvaultmv126052023"
  location            = data.azurerm_resource_group.rg.location
  resource_group_name = data.azurerm_resource_group.rg.name
...
data "azurerm_key_vault_secret" "publicclave" {
  name         = "publicclave"
  key_vault_id = azurerm_key_vault.kvaultmv126052023.id
}
