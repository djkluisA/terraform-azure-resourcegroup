...
11. Conecta el bastion host a la red virtual utilizando el atributo 'network_interface_ids' con el id de la interfaz de red creada anteriormente.

12. Configura el 'admin_username' de la máquina virtual Linux como azureuser y utiliza un bloque 'admin_ssh_key' para obtener la clave pública desde el key vault.

13. Asegúrate de que todos los recursos estén ubicados en el grupo de recursos especificado utilizando el recurso 'azurerm_resource_group' y el atributo 'name'.

14. Configura el proveedor de Azure con el atributo 'skip_provider_registration' en 'true' y el bloque 'features'.

15. Ejecuta el código en Terraform cloud con el proveedor Azure.

Espero que esto te ayude a comenzar con tu despliegue en Azure Cloud utilizando Terraform. Recuerda que este es solo un ejemplo general y que debes adaptarlo a tus necesidades específicas.